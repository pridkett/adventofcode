...
class Solution:
    ...
    def _evaluate(self, rep: List[int]) -> int:
        asc = rep[1] > rep[0]
    ...
    def evaluate_new(self, report: str) -> Tuple[int, int]:
        rep = [int(x) for x in report.strip().split()]
        rv1 =  self._evaluate(rep)
        if rv1 == 1:
            return (1,1)

        # brute force this
        rv2 = min(sum([self._evaluate(rep[:x]+rep[x+1:]) for x in range(0, len(rep))]), 1)
        return rv1, rv2
